...
- **Reset file** button at the sidebar footer (next to the active table name) drops the loaded CSV and clears it from IndexedDB. Chart config (layers / labels / settings) stays — re-upload a compatible file and keep your layout. Hover the button for an instant tooltip with the same explanation.
- **Persisted file.** Uploaded CSVs are saved to IndexedDB (single record, last-upload-wins). On page reload the bytes are re-registered with ggsql before plotr restores `activeTable` from the URL hash — your file survives reloads transparently. Panel selection survives reloads too: whichever side panel (chart layer / labels / custom / shared) was open and whichever secondary panel (settings / mapping) was open re-open on the next visit. Stale ids (e.g. you deleted the previously-selected layer) silently fall back to the first-chart-layer auto-open default. "Reset file" is the only way to drop it; private-mode / quota / IDB-missing failures fall back to per-session-only behavior with a console warning. Browser **Back** / **Forward** step through previous chart edits — dropping a variable, typing a label, switching dataset, picking a chart type, etc. Rapid edits within 600 ms coalesce into a single history entry so a burst of typing in the Labels field is one step rather than per-keystroke. Pure UI navigation (clicking a layer card or the settings cog to open / switch the side panel) doesn't add history entries — only the URL is updated silently, so Back never just closes a panel.
- **Build pane (middle).** Photoshop-style narrow icon column.
  ...
  - **Reset chart** button anchored at the bottom of the rail clears all layers, labels, custom blocks, and shared settings. The loaded file (`activeTable`) is preserved. Hover for an instant tooltip with the same explanation. Same icon as the data-pane "Reset file" button; the two are deliberately split so file vs chart can be reset independently.
